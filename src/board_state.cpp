...
BoardState::BoardState(const std::string& turn, const std::string& castlingRights,
                       const std::string& enpassant) noexcept
    : state_{}
{
    setTurn(turn);
    setCastlingPrivileges(castlingRights);
    setEnpassantSquare(enpassant == "-" ? std::nullopt
...
{
    if ((color == "b") != blackToMove())
        updateTurn(); // toggle turn due to mismatch
}
...
std::string BoardState::parseTurn() const noexcept { return blackToMove() ? "b" : "w"; }
...
bool BoardState::operator!=(const BoardState& other) const noexcept { return !(operator==(other)); }
