...
void simulateSelfPlay(const SelfPlayArgs& args, const char* exePath)
{
    ...
    std::cout << "Simulation complete!\n\n"
              ...
              << " secs\n"
              << "Nodes searched:   " << std::setw(12) << engine.getNodesSearchedCount()
              << " nodes\n"
              << "Nodes per second: " << std::setw(12)
              << engine.getNodesSearchedCount() * 1000 / duration << " nps\n\n";
}
...
void printBestMove(int argc, const char** argv)
{
    ...
    if (argc == 5 && (std::string(argv[3]) == "-d" || std::string(argv[3]) == "--depth"))
    {
        ...
        std::cout << std::visit([](auto&& arg) { return arg.uci(); },
                                engine.search(std::stoi(argv[4])))
                  << '\n';
    }
    else if (argc == 3)
    {
...
int main(int argc, const char* argv[])
{
    ...
    // simulate self-play with variable number of moves and variable search depth from default
    // position
    if (argc <= 10 && argc % 2 == 0 && std::string(argv[1]) == "--simulate")
    {
        simulateSelfPlay(parseSelfPlayArgs(argc, argv),
                         argv[0]); // defaults to simulation of 100 moves at depth 6
        return 0;
    }
