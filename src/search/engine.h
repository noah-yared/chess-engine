...
class SearchEngine
{
  private:
    ...
    explicit SearchEngine(const Position& position, size_t ttSize) noexcept
        : position_{position}, undoStack_{}, tt_{ttSize}, nodesSearched_{0ULL}, whiteHistory_{},
          blackHistory_{}, moveBuffer_{} {};
    ...
    bool isEasyMode(Difficulty difficulty) noexcept { return difficulty <= Difficulty::NOVICE; }
    ...
    int getTTScoreOrSearch(int& alpha, int& beta, int depth, int originalDepth) noexcept
    {
        ...
        return alphaBeta<opposite<color>(), easyMode>(alpha, beta, depth - 1, originalDepth).score;
    }
    ...
    Line alphaBeta(int alpha, int beta, int depth, int originalDepth = DEFAULT_SEARCH_DEPTH) noexcept
    {
        ...
        {
            int finalEval =
                easyMode ? position_.evaluation<true>() : quiescenceSearch<color>(alpha, beta);
        ...
        for (const auto move : possibleMoves)
        {
            ...
            int eval = getTTScoreOrSearch<color, easyMode>(alpha, beta, depth, originalDepth);
            backtrack(move);
  ...
  public:
    SearchEngine() noexcept
        : position_{}, undoStack_{}, tt_{}, nodesSearched_{0ULL}, whiteHistory_{}, blackHistory_{},
          moveBuffer_{} {};
    explicit SearchEngine(const std::string& fen) noexcept
        : position_{fen}, undoStack_{}, tt_{}, nodesSearched_{0ULL}, whiteHistory_{},
          blackHistory_{}, moveBuffer_{} {};
    ...
    MoveVariant search(int depth = DEFAULT_SEARCH_DEPTH) noexcept
    {
        ...
        auto bestMoveFound = alphaBeta<color, easyMode>(NEGINF, POSINF, depth).move;

        assert(bestMoveFound && "No move was found!");

        advance(*bestMoveFound);
        updateHistory<opposite<color>()>(position_);
        return *bestMoveFound;
    }

    MoveVariant search(int depth = DEFAULT_SEARCH_DEPTH,
                       std::optional<Difficulty> difficulty = {}) noexcept
    {
        if (difficulty.has_value() && isEasyMode(*difficulty))
        {
            return turn() == Color::WHITE ? search<Color::WHITE, true>(depth)
                                          : search<Color::BLACK, true>(depth);
        }

        return turn() == Color::WHITE ? search<Color::WHITE, false>(depth)
                                      : search<Color::BLACK, false>(depth);
    }
