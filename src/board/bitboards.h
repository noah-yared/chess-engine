...
class Bitboards
{
    ...
    {
        return (first == last) ? 0ULL
                               : std::accumulate(first, last, 0ULL,
                                                 [](u64 acc, u64 bb) noexcept { return acc | bb; });
    }
    ...
    {
        auto found =
            std::find_if(first, last, [mask](u64 bb) noexcept -> bool { return bb & mask; });
        return found != last ? PieceType(std::distance(first, found) % NUM_PIECE_TYPES)
                             : PieceType::NONE;
    }

    [[nodiscard]] int wKing() const noexcept
    {
        return BitUtils::ctz(bb(PieceType::KING, Color::WHITE));
    }
    [[nodiscard]] int bKing() const noexcept
    {
        return BitUtils::ctz(bb(PieceType::KING, Color::BLACK));
    }
  ...
  public:
    ...
    Bitboards() noexcept
        : bbs_(STARTING_BBS), whiteBB_(STARTING_WHITE_BB), blackBB_(STARTING_BLACK_BB) {};
    ...
    static Color keyToColor(int key) noexcept { return Color(key >= NUM_PIECE_TYPES); }
    ...
    void togglePieceSquare(int key, int square) noexcept
    {
        ...
        keyToColor(key) == Color::WHITE ? (whiteBB_ ^= 1ULL << square)
    ...
    [[nodiscard]] iterator bEnd() const noexcept
    {
        return std::next(bbs_.begin(), NUM_PIECE_TYPES);
    }
    ...
    [[nodiscard]] u64 allyBB(Color color) const noexcept
    {
        return color == Color::WHITE ? whiteBB_ : blackBB_;
    }
    [[nodiscard]] u64 opposingBB(Color color) const noexcept
    {
        return color == Color::WHITE ? blackBB_ : whiteBB_;
    }
    ...
    [[nodiscard]] PieceType getPieceType(int square, Color color) const noexcept
    {
        if (u64 pieceMask = 1ULL << square; allyBB(color) & pieceMask)
            return color == Color::WHITE ? findPiece(wStart(), wEnd(), pieceMask)
    ...
    [[nodiscard]] bool operator==(const Bitboards& other) const noexcept
    {
        return bbs_ == other.bbs_;
    }
    [[nodiscard]] bool operator!=(const Bitboards& other) const noexcept
    {
        return bbs_ != other.bbs_;
