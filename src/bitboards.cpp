...
Bitboards::Bitboards(const std::string& sBoard, FromAsciiBoard) noexcept
    : bbs_{}, whiteBB_{}, blackBB_{}
{
    ...
    assert(bit == -1 &&
           "Invalid ascii board format passed in! Make sure to specify all squares on the board!");
}
...
Bitboards::Bitboards(const std::string& fen, FromFEN) noexcept : bbs_{}, whiteBB_{}, blackBB_{}
{
    ...
    for (auto c : fenPieces)
    {
        ...
        if (isdigit(c))
            ...
            togglePieceSquare(static_cast<int>(pieceString.find(c)), bit--);
    }
}
...
{
    const std::string pieceString = "prnbqkPRNBQK";
    if (whiteBB_ & blackBB_)
        return false;
    if (combine(wStart(), wEnd()) != whiteBB_)
        return false;
    if (combine(bStart(), bEnd()) != blackBB_)
        return false;
    for (int i = 0; i < NUM_BITBOARDS; ++i)
    {
        for (int j = i + 1; j < NUM_BITBOARDS; ++j)
        {
            if (bbs_[i] & bbs_[j])
            {
                std::cout << pieceString[i] << "'s bitboard and " << pieceString[j]
                          << "'s bitboard overlap!" << std::endl;
                return false;
            }
        }
    }
