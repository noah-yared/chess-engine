...
public:
  ...
  static constexpr inline ResultType accumulateBits(u64 bb, Accumulator accumulator, ResultType init = {}) noexcept {
  ...
  static constexpr inline u64 clearFile(u64 bb) noexcept { return bb & ~fileMask<file>(); }
  template<char file> requires IsFile<file>
  static constexpr inline u64 filterFile(u64 bb) noexcept { return bb & fileMask<file>(); }
  template<int rank> requires IsRank<rank>
  static constexpr inline u64 clearRank(u64 bb) noexcept { return bb & ~rankMask<rank>(); }
  template<int rank> requires IsRank<rank>
  static constexpr inline u64 filterRank(u64 bb) noexcept { return bb & rankMask<rank>(); }
...
private:
  ...
  static constexpr inline u64 sft(u64 bb, int sfamt) noexcept { return sfamt > 0 ? bb << sfamt : bb >> -sfamt; }

  template<char file> requires IsFile<file>
  static constexpr u64 fileMask() noexcept { return LeftEdgeMask >> (file - 'a'); }

  template<int rank> requires IsRank<rank>
  static constexpr u64 rankMask() noexcept { return BottomEdgeMask << (8 * (rank - 1));  }
