...
struct MoveBase
{
  public:
    ...
    MoveBase(unsigned int start, unsigned int end, Color side, PieceType moved,
             std::optional<PieceType> captured = std::nullopt) noexcept
        : start_(start), end_(end), side_(static_cast<unsigned int>(side)),
          moved_(static_cast<unsigned int>(moved)), isCapture_(captured.has_value()),
          captured_(static_cast<unsigned int>(captured.value_or(PieceType::NONE))) {};

  private:
    unsigned int start_ : 6, end_ : 6, side_ : 1, moved_ : 3, isCapture_ : 1, captured_ : 3;
  ...
  public:
    ...
    [[nodiscard]] std::optional<int> capturedKey() const noexcept
    {
        return isCapture_ ? std::optional<int>(Bitboards::pieceToKey(captured(), oppSide()))
                          : std::nullopt;
    }
    ...
    [[nodiscard]] std::string uci() const noexcept
    {
        ...
        if (mType == MoveType::Promotion)
            ss << 'q'; // always promote to queen
        return ss.str();
    }
    ...
    {
        return start() == other.start() && end() == other.end() && moved() == other.moved() &&
               captured() == other.captured() && mType == OtherMoveType;
    }
    template <MoveType OtherMoveType>
    bool operator!=(const MoveBase<OtherMoveType> other) const noexcept
    {
...
struct Move : public MoveBase<mType>
{
    ...
    Move(unsigned int start, unsigned int end, Color side, PieceType moved,
         std::optional<PieceType> captured = std::nullopt) noexcept
...
struct Move<MoveType::Promotion> : public MoveBase<MoveType::Promotion>
{
    ...
    [[nodiscard]] int promotionKey() const noexcept
    {
        return Bitboards::pieceToKey(PieceType::QUEEN, side());
    }
};
...
struct Move<MoveType::Castle> : public MoveBase<MoveType::Castle>
{
    ...
    [[nodiscard]] int castledRookKey() const noexcept
    {
        return Bitboards::pieceToKey(PieceType::ROOK, side());
    }
};
...
struct Move<MoveType::Enpassant> : public MoveBase<MoveType::Enpassant>
{
    ...
    [[nodiscard]] int enpassantKey() const noexcept
    {
        return Bitboards::pieceToKey(PieceType::PAWN, oppSide());
    }
};
...
struct Move<MoveType::DoublePawnPush> : public MoveBase<MoveType::DoublePawnPush>
{
    ...
    [[nodiscard]] int enpassantKey() const noexcept
    {
        return Bitboards::pieceToKey(PieceType::PAWN, side());
    }
};
...
struct hash<Move<mType>>
{
    ...
    {
        return move.start() ^ move.end() ^ std::hash<PieceType>()(move.moved()) << 2 ^
               std::hash<PieceType>()(move.captured());
    }
};
} // namespace std
