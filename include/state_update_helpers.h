...
template<>
inline void updateEnpassantSquare<MoveType::DoublePawnPush>(const Move<MoveType::DoublePawnPush> move, BoardState& state) noexcept {
...
template<>
inline void updateCastlingPrivs<MoveType::Castle>(const Move<MoveType::Castle> move, BoardState& state) noexcept {
  move.side() == Color::WHITE ? state.stripCastlingPrivileges<'K', 'Q'>() : state.stripCastlingPrivileges<'k', 'q'>();
}
