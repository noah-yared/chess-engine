...
class BoardState {
  ...
  static constexpr u32 encode(const std::vector<char>& castlingRights, Color playerTurn, std::optional<int> enpassantSq) noexcept {
    u32 state = 0;
  ...
  [[nodiscard]] bool existsEnpassantSq() const noexcept { return (state_ >> BIT_OFFSETS::EXISTS_ENPASSANT_SQ) & 0x1; }
  [[nodiscard]] int enpassantSq() const noexcept { return (state_ >> BIT_OFFSETS::ENPASSANT_SQ) & 0x3f; }
...
public:
  ...
  BoardState(const std::vector<char>& castlingRights, Color playerTurn, std::optional<int> enpassantSq) noexcept : 
    state_{encode(castlingRights, playerTurn, enpassantSq)} {};

  BoardState(const std::string& turn, const std::string& castlingRights, const std::string& enpassant) noexcept;
  BoardState(const std::string& fen) noexcept;

  // Simple accessors
  [[nodiscard]] int castlingBits() const noexcept { return (state_ >> BIT_OFFSETS::CASTLING_PRIVS) & 0xf; }
  [[nodiscard]] bool isCastlingRightAvailable(char right) const noexcept { return castlingBits() & (1 << PRIV_BIT_OFFSET(right)); }
  ...
  [[nodiscard]] std::optional<int> getEnpassantSquare() const noexcept { return existsEnpassantSq() ? std::optional<int>(enpassantSq()) : std::nullopt; }
