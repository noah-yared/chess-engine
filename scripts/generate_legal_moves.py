...
def get_ucis(fen: str|None = None, promo_to_queen_only=False) -> list[str]:
  ...
  return list({ move.uci() for move in board.legal_moves if move.uci()[-1] not in "nbr" })

def stringify_ucis(fen: str|None = None, ucis_per_row: int = 10, promo_to_queen_only=False) -> str:
  ucis_str = ""
  uci_list = list(sorted(get_ucis(fen=fen, promo_to_queen_only=promo_to_queen_only))) # deterministic output
  for row in range(math.ceil(len(uci_list) / ucis_per_row)):
    ucis_str += '      '
    for i in range(row * ucis_per_row, min((row + 1) * ucis_per_row, len(uci_list))):
      ucis_str += (f'"{uci_list[i]}", ' if i != len(uci_list) - 1 else f'"{uci_list[i]}"')
    ucis_str += '\n'
  return ucis_str[:-1]
