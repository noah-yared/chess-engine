...
def get_ucis(fen: str | None = None) -> list[str]:
    board = chess.Board() if fen is None else chess.Board(fen)
    return [move.uci() for move in board.legal_moves]


def stringify_ucis(fen: str | None = None, ucis_per_row: int = 10) -> str:
    ucis_str = ""
    # sort to get deterministic output
    uci_list = list(sorted(get_ucis(fen=fen)))
...
if __name__ == "__main__":
    ...
    stringified_ucis = stringify_ucis(fen=sys.argv[1])
