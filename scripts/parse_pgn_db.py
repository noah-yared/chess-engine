...
def write_unique_fens_to_file_from_compressed_pgns(
    input_file_path: str,
    output_file_path: str,
    max_fens_to_process: int = 1_000_000,
    max_fens_to_write: int = 1_000_000,
) -> None:
    with open(output_file_path, "w") as output_file:
        ...
        truncate_fen_proc = subprocess.Popen(
            ["head", "-n", str(max_fens_to_write)],
            stdin=unique_fen_proc.stdout,
            stdout=output_file,
            text=True,
            encoding="utf-8",
        )
...
if __name__ == "__main__":
    write_unique_fens_to_file_from_compressed_pgns(
        COMPRESSED_PGN_FILE_PATH, OUTPUT_FEN_FILE_PATH
    )
